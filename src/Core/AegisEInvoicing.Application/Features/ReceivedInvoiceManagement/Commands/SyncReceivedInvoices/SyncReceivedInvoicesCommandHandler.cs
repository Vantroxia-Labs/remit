...
/// <summary>
/// Handler for synchronizing received invoices from Interswitch
/// Creates new invoices or updates existing ones with the latest data from Interswitch
/// </summary>
public sealed class SyncReceivedInvoicesCommandHandler : IRequestHandler<SyncReceivedInvoicesCommand, SyncReceivedInvoicesResult>
{
        ...
        ILogger<SyncReceivedInvoicesCommandHandler> logger)
    {
        ...
        _interswitchClient = interswitchClient ?? throw new ArgumentNullException(nameof(interswitchClient));
    ...
    public async Task<SyncReceivedInvoicesResult> Handle(SyncReceivedInvoicesCommand request, CancellationToken cancellationToken)
    {
        try
        {
            var userId = _currentUser.UserId ?? Guid.Parse("9c17ea5c-483c-44f8-97e8-c364e6739949");
                ...
                // Call Interswitch API to get purchase invoices
                var getPurchaseInvoicesRequest = new GetPurchaseInvoicesRequest
                {
                ...
                _logger.LogInformation(
                    "Calling Interswitch API for purchase invoices with TIN {TIN}",
                    business.TaxIdentificationNumber.Value);

                var response = await _interswitchClient.GetPurchaseInvoicesAsync(
                ...
                _logger.LogInformation(
                    "Found {Count} invoices from Interswitch for business {BusinessId}",
                    response.Count, request.BusinessId);

                // Get existing invoices to check for updates
                var invoiceIRNs = response.Data.Select(i => i.IRN).ToList();
                    ...
                    try
                    {
                        ...
                        if (existingInvoiceDict.TryGetValue(item.IRN, out var existingInvoice))
                        {
                            ...
                            existingInvoice.UpdateAllFromSync(
                                ...
                                lineExtensionAmount: item.LegalMonetaryTotal!.LineExtensionAmount!.Value,
                                taxExclusiveAmount: item.LegalMonetaryTotal!.TaxExclusiveAmount!.Value,
                                taxInclusiveAmount: item.LegalMonetaryTotal!.TaxInclusiveAmount!.Value,
                                totalTaxAmount: item.LegalMonetaryTotal!.TaxInclusiveAmount!.Value - item.LegalMonetaryTotal!.TaxExclusiveAmount!.Value,
                                payableAmount: item.LegalMonetaryTotal!.PayableAmount!.Value,
                                updatedBy: userId,
                        ...
                        var confirmInvoice = await _interswitchClient.ConfirmInvoiceAsync(new Interswitch.Models.Requests.ConfirmInvoice.ConfirmInvoiceRequest
                        {
                             IRN = item.IRN
                        });
                    }
